...
:::tip
如果你不需要 D3 的底层控制力,我们推荐它更高层级[的姐妹库:Observable Plot](https://observablehq.com/plot)。在 D3 中可能要写 50 行代码的直方图,Plot 一行搞定!Plot 简洁而富有表现力的 API 让你把精力放在分析和可视化数据上,而不是 Web 开发上。你甚至可以把 Plot 和 D3 结合使用,兼得两者之长。
:::
